...
## Content Calendar (`pages/calendar.tsx`, `content_calendar_days`/`content_calendar_events`)

Shows two independent kinds of chips on the same monthly grid: manual social-post events (`content_calendar_events`, created/edited via the day modal — see `supabase/migrations/0006_content_calendar.sql`, a table deliberately separate from the blog pipeline) and blog posts. The blog-post chips read from **both** `/api/scheduled-articles` and `/api/published-articles` merged client-side, not from `content_calendar_events` — a blog post never gets written into that table.

**Fixed 2026-08-27**: the calendar used to only fetch `/api/scheduled-articles`, which by design only returns `status = "scheduled"` rows (that same endpoint also backs the Scheduled page, which must never show already-published articles). The moment a post actually published, it fell out of that query and its chip vanished from the calendar entirely — exactly the "blog posts aren't updating" symptom. Fixed by merging in `/api/published-articles` too (now also selecting `image_url`, with the same optional-column fallback as everywhere else), tagging each row with `status: "scheduled" | "published"`, and giving published posts a distinct chip color (`bg-green-700` vs. the scheduled `BLOG_POST_COLOR_CLASS`) — see the updated legend. Clicking a published post's day now links to its `/published/[id]/stats` page instead of the wizard's edit view.
