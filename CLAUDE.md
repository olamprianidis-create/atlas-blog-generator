...
- **Auto-deleted 7 days after a successful publish** (`utils/cleanupOldUploads.ts`, added 2026-09-03, run as the last step of the same daily cron as scheduled publishing) — a `video_uploads` row (and its Blob-stored video/thumbnail files) is deleted once `published_at` is 7+ days old **and** every platform it targeted actually reached `"published"` (not `"failed"`/`"not_connected"` — a failed publish's source file is deliberately kept, since it may still be needed for a retry). This is a full row deletion, not just the file — the entry disappears from the Uploads page entirely, by explicit request. Each Blob delete uses the row's own `video_url`/`thumbnail_url` directly (never a fuzzy match against a separate storage listing) — see the 2026-09-02 incident note below for why that distinction matters.
...
- **Connected as of 2026-08-30** — a real LinkedIn app is set up and credentials are live; confirmed working via a real successful post.
- **`LINKEDIN_API_VERSION` needs periodic bumping** (`utils/linkedin.ts`) — LinkedIn sunsets each monthly version ~1 year after release, and a sunset version fails every request with a `426 NONEXISTENT_VERSION` error. Hit this exact bug 2026-08-30 (was pinned to `202505`, long sunset) — fixed by bumping to the then-latest `202608`. Check LinkedIn's versioning docs for the current latest if this error reappears.
- **Best-effort, never blocks a publish** — the auto-share block in `publishArticleById()` (`utils/publish.ts`) catches every error itself; a LinkedIn failure only sets `scheduled_articles.linkedin_status`/`linkedin_error`, it never fails the publish (`publishArticleById()`'s return value only reflects whether the article itself published). The Published page (`pages/published.tsx`) shows that status per article plus a manual "Share to LinkedIn" button (`POST /api/published-articles/[id]/linkedin`) — used for articles published before LinkedIn was connected, or to retry a failure. **Connecting/reconnecting LinkedIn itself happens from the bottom of Step 5 in the generator** (moved 2026-08-28, `components/steps/StepFiveSchedule.tsx`'s `LinkedInSection`), not the Published page.
- **Per-article opt-out (`linkedin_auto_share`, `0015_linkedin_auto_share.sql`, added 2026-08-30)** — every article defaults to auto-sharing (matches the behavior every article had before this column existed), but Step 5's LinkedIn box now has a checkbox to turn it off per article. `publishArticleById()` skips the LinkedIn call entirely when it's false, leaving `linkedin_status` at its `"not_posted"` default. Toggled via `PATCH /api/scheduled-articles/[id]` (`{ linkedinAutoShare: boolean }`), independent of the full re-schedule flow in `pages/api/schedule-article.ts`.
- **Tokens expire in 60 days with no refresh token by default** — LinkedIn's standard OAuth doesn't issue a refresh token unless you separately apply for its "Programmatic Refresh Tokens" product. Reconnecting periodically via Step 5's "Connect" button is the expected flow otherwise; an expired connection surfaces as `linkedin_status: "not_connected"` on the next publish, same as never having connected.
- Reuses the `platform_connections` table from the Uploads feature (`supabase/migrations/0008_linkedin.sql` widens its `platform` check constraint to include `'linkedin'`, and adds `linkedin_status`/`linkedin_post_urn`/`linkedin_error`/`linkedin_posted_at` to `scheduled_articles`) — confirmed run.
...
**Platform list note**: `utils/types.ts`'s `Platform`/`PLATFORMS` no longer includes `facebook` (dropped 2026-08-28) and now includes `blog` (added 2026-08-30, teal — same `BLOG_POST_COLOR_CLASS` as real blog article chips, so a note can be tagged "Blog" too) alongside Instagram/YouTube/TikTok/Pinterest. Never included `linkedin` (LinkedIn is blog-article-only, see below). If a platform is asked for again, add it to that one array (used by the event `PlatformPicker`, the calendar legend, and chip coloring).

**Notes redesign (2026-08-30)**: manual platform events are now called "Notes" in the UI. Each has a short **header** (`title` column, `0013_calendar_event_title.sql`) shown on the main grid, separate from a longer rich-text **description** (`NoteRichTextEditor` — stores HTML, bold/italic/underline/highlight/lists — only shown once the day is opened; note this is a *different* component from `components/RichTextEditor.tsx`, which serializes to the Step 3 outline's plain-text format — don't reuse one for the other, their value contracts are incompatible). Notes render as yellow (`bg-amber-300`) squares pinned to the bottom of each day cell, full-width and `h-16` tall, stacked one per note — separate from the top scrollable area where blog/video/Google Calendar chips live. Tagged platforms show as small colored dots inside the note square rather than separate chips. The note's checkbox marks all of its tagged platforms done/not-done together (`toggleNoteCompleted()` in `pages/calendar.tsx`), unlike before when each platform got its own separate chip+checkbox.

**Google Calendar two-way sync (2026-08-30)** — `utils/googleCalendar.ts`, `pages/api/google-calendar/*`, `pages/api/auth/google-calendar/*`:
- Reuses the **same OAuth client as YouTube** (`YOUTUBE_CLIENT_ID`/`YOUTUBE_CLIENT_SECRET`) since it's the same Google Cloud project issuing tokens for a different Google API — just enable "Google Calendar API" in that project and add `${APP_URL}/api/auth/google-calendar/callback` as a second authorized redirect URI on the existing OAuth client.
- Connect via the card at the top of `/calendar`, then pick which calendar to sync from the dropdown (one Google account can have many calendars — `platform_connections.account_label` stores the chosen `calendarId`).
- **Pull (read)**: `/api/google-calendar/events?year&month` fetches events live from Google on every page load (no local copy kept) and renders them as indigo "Google" chips — read-only, no checkbox. Events that were pushed there by our own synced notes are excluded from this pull (matched by `google_event_id`) so they don't render twice.
- **Push (write)**: each note has an "Also add to Google Calendar" checkbox (only shown once connected). Checking it creates/updates a mirrored Google event and stores its id in `content_calendar_events.google_event_id`; unchecking it deletes that Google event. Handled in `pages/api/calendar/events.ts` (POST) and `pages/api/calendar/events/[id].ts` (PATCH/DELETE) — best-effort, wrapped in try/catch so a Google API hiccup never loses the note itself, which is always saved locally first.
- New optional columns (`sync_to_google_calendar`, `google_event_id` — `0014_google_calendar_and_discord.sql`) follow the same graceful-degradation SELECT-tiering pattern as `title`/`completed_platforms`.

**Discord (2026-08-30)** — `utils/discord.ts`, `pages/discord.tsx`, `pages/api/discord/*`:
- Bot-token auth (`DISCORD_BOT_TOKEN` env var), not per-user OAuth — there's no "Connect" flow, just the bot needing to already be invited into the server. `isDiscordConfigured()` just checks the env var is set.
- `discord_channels` table (`0014_google_calendar_and_discord.sql`) is a saved address book of `{label, channel_id, is_default}` so the admin doesn't have to paste a raw channel ID every time; exactly one row can be `is_default` (enforced in application code, not a DB constraint), which is where auto-posts go.
- **Manual**: the `/discord` page has a compose box — pick a saved channel or paste an ID, write a message, hit Send (`POST /api/discord/post`).
- **Automatic**: `autoPostToDiscord()` fires from `utils/publish.ts` (blog article publish) and `utils/publishVideo.ts` (YouTube/TikTok video publish) — best-effort/non-blocking, silently does nothing if Discord isn't configured or no default channel is set, same "shouldn't block the real action" pattern as the Google Calendar sync above.
- **Weekly call reminders (2026-09-03)** — `pages/api/cron/weekly-reminder.ts` posts to a fixed channel (`REMINDER_CHANNEL_ID` in that file, currently the "main" channel under the Q3 2026 category — not one of the `discord_channels` saved addresses, since it's a fixed target rather than admin-picked) at 5:30 PM PT ("starts in 30 minutes", includes the Zoom link — read live from the Website's `WeeklyMeeting` table via `getWeeklyMeeting()` in `utils/websiteDb.ts`) and 6:00 PM PT ("we're live", no link — deliberately, two links in ten minutes was called overkill). Not a Vercel Cron entry — piggybacks on the existing every-10-minutes GitHub Actions ping in `.github/workflows/publish-cron.yml` (originally added for scheduled publishing, since Vercel's Hobby-plan cron only guarantees firing "sometime within the hour," useless for a precise 5:30/6:00 pair). The endpoint checks the *real* current Pacific time itself (`Intl.DateTimeFormat` with `timeZone: "America/Los_Angeles"`) and no-ops outside a 10-minute window around each target time, so it's automatically correct across the twice-yearly DST shift — no fixed-UTC cron expression to manually adjust in November/March.

## Platform setup checklist (YouTube, TikTok, LinkedIn, Google Calendar, Discord)

None of these auto-posting integrations work until their developer-portal app exists and its credentials are in `.env.local`/Vercel. This is a one-time setup Claude can't do on your behalf (each requires logging into a developer console with your own account):

1. **Apply the pending migrations** — `supabase/migrations/0007_uploads.sql`, `0008_linkedin.sql`, and `0014_google_calendar_and_discord.sql`, run manually in the Supabase SQL editor (Supabase project → SQL Editor → paste → Run). Nothing below works without these tables/columns existing.
...
4. **Google Calendar**: same Google Cloud project/OAuth client as YouTube above — just also enable "Google Calendar API" (APIs & Services → Library) and add `${APP_URL}/api/auth/google-calendar/callback` as a *second* Authorized redirect URI on that same OAuth client. No new env vars needed.
5. **TikTok**: [TikTok for Developers](https://developers.tiktok.com) → create an app → add the "Content Posting API" product → Manage apps → add `${APP_URL}/api/auth/tiktok/callback` as a redirect URI → verify `APP_URL`'s domain (Content Posting API settings — required for `PULL_FROM_URL`) → copy the client key/secret into `TIKTOK_CLIENT_KEY`/`TIKTOK_CLIENT_SECRET`. Submit for app audit when ready to post publicly (not required to test against your own sandboxed account first).
6. **LinkedIn**: [LinkedIn Developer Portal](https://www.linkedin.com/developers/apps) → create an app (needs an associated LinkedIn Company Page, even though posts go to your personal profile — LinkedIn requires this to create any app) → Products tab → add both "Sign In with LinkedIn using OpenID Connect" and "Share on LinkedIn" (both self-serve, no review) → Auth tab → add `${APP_URL}/api/auth/linkedin/callback` under Authorized redirect URLs → copy the Client ID/Secret into `LINKEDIN_CLIENT_ID`/`LINKEDIN_CLIENT_SECRET`.
7. **Discord**: [Discord Developer Portal](https://discord.com/developers/applications) → new application → Bot tab → add bot, copy its token into `DISCORD_BOT_TOKEN` → OAuth2 → URL Generator → scope `bot`, permission `Send Messages` → open the generated URL to invite the bot into your server.
8. Deploy with the new env vars set, then click each platform's "Connect" button (YouTube/TikTok on the Uploads page, LinkedIn on Step 5 of the generator, Google Calendar at the top of the Content Calendar page) and complete that platform's OAuth consent screen once. Discord has no Connect step — add its bot token and invite the bot, then add channels on the `/discord` page.
