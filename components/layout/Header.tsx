...
const STATISTICS_LINKS = [
  { href: "/statistics/applicants", label: "Applicants" },
  { href: "/statistics/survey", label: "New Member Survey" },
];
...
export default function Header() {
  ...
  const isStatisticsActive = STATISTICS_LINKS.some((link) => link.href === router.pathname);
    ...
    <header className="h-16 shrink-0 border-b border-slate-200 bg-white px-6 flex items-center justify-between">
      ...
      <nav className="flex items-center gap-6">
        ...
        <HoverDropdown
          ...
          trigger={
            <Link
              href="/statistics/applicants"
              className={`${NAV_LINK_CLASSES} ${isStatisticsActive ? "text-slate-900" : "text-slate-600 hover:text-slate-900"}`}
            >
              Statistics
            </Link>
          }
